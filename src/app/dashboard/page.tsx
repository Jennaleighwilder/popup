...
export default function DashboardPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#2D2420] flex items-center justify-center">
        <div className="w-10 h-px bg-[#C4A574] animate-pulse" />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#2D2420]">
      <nav className="border-b border-[#5C4033] px-6 py-5">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link href="/" className="font-[family-name:var(--font-display)] text-2xl font-light text-[#F5EDE4]">
            Popup
          </Link>
          <div className="flex items-center gap-6">
            ...
            <Link
              href="/create"
              className="flex items-center gap-2 px-5 py-2.5 bg-[#8B2500] text-white text-sm tracking-wider uppercase hover:bg-[#6B1D00] transition-colors"
            >
      ...
      <main className="max-w-6xl mx-auto px-6 py-16">
        <h1 className="font-[family-name:var(--font-display)] text-3xl font-light text-[#F5EDE4] mb-12">
          Your events
        </h1>
        ...
        {loadingEvents ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="border border-[#5C4033] bg-white p-6 h-64 animate-pulse" />
            ))}
          </div>
        ) : events.length === 0 ? (
          <div className="border border-[#5C4033] p-16 text-center bg-white">
            <p className="font-[family-name:var(--font-body)] text-[#8C8578] mb-8">
              No events yet. Create your first one to get started.
            </p>
            <Link
              href="/create"
              className="inline-flex items-center gap-2 px-8 py-4 bg-[#8B2500] text-white font-[family-name:var(--font-body)] text-sm tracking-wider uppercase hover:bg-[#6B1D00] transition-colors"
            >
              <Plus className="w-4 h-4" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            {events.map((ev, i) => (
              <motion.div
                ...
                className="border border-[#5C4033] bg-white overflow-hidden group"
              >
                ...
                <div className="p-6">
                  <h2 className="font-[family-name:var(--font-display)] text-xl font-light text-[#F5EDE4] mb-2 line-clamp-2">
                    {ev.name}
                  </h2>
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Link
                      href={`/edit/${ev.slug}`}
                      className="flex items-center gap-1.5 px-3 py-2 border border-[#5C4033] font-[family-name:var(--font-body)] text-sm hover:border-[#C4956A] transition-colors"
                    >
        ...
        {!demoMode && user && (
          <section className="border-t border-[#5C4033] pt-16 mt-16">
            <h2 className="font-[family-name:var(--font-display)] text-2xl font-light text-[#F5EDE4] mb-6">
              Account
            </h2>
            <div className="grid md:grid-cols-2 gap-8">
              <div className="p-6 border border-[#5C4033] bg-white">
                <h3 className="font-[family-name:var(--font-mono)] text-xs text-[#8C8578] uppercase tracking-wider mb-4">
                  Profile
                </h3>
                <p className="font-[family-name:var(--font-body)] text-[#F5EDE4]">
              ...
              <div className="p-6 border border-[#5C4033] bg-white">
                ...
                <p className="font-[family-name:var(--font-body)] text-[#F5EDE4] mb-4">
